...
def _query_prom_ratio(prom_url: str, window: int) -> float | None:
    expr = (
        "sum(rate(http_requests_total{status=~\"5..\"}[{w}s])) / "
        "clamp_min(sum(rate(http_requests_total[{w}s])), 1)"
    ).format(w=int(window))
...
def main() -> int:
    ...
    parser.add_argument("--prom-url", type=str, default=os.getenv("PROM_QUERY_URL") or "")
