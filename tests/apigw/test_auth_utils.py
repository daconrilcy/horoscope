...
class TestExtractTenantSecure:
    ...
    def test_internal_header_allowed(self) -> None:
        ...
        request = self.create_mock_request(
            headers={
                "X-Tenant-ID": "internal_tenant",
                "X-Internal-Auth": "signed_token"
            }
        )
...
class TestInternalTrafficDetection:
    ...
    def test_internal_auth_header(self) -> None:
        ...
        request = self.create_mock_request(
            headers={"X-Internal-Auth": "signed_token"}
        )
        
        assert _is_internal_traffic(request) is True

    def test_service_mesh_header(self) -> None:
        """Test détection via X-Service-Mesh."""
        request = self.create_mock_request(
            headers={"X-Service-Mesh": "internal"}
        )
...
class TestIntegration:
    ...
    async def test_middleware_uses_trust_model(self) -> None:
        ...
        with patch("backend.apigw.rate_limit.extract_tenant_secure") as mock_extract, \
            ...
            from backend.apigw.rate_limit import TenantRateLimitMiddleware
    ...
    async def test_quota_middleware_uses_trust_model(self) -> None:
        ...
        with patch("backend.apigw.rate_limit.extract_tenant_secure") as mock_extract, \
            ...
            from backend.apigw.rate_limit import QuotaMiddleware
            ...
            mock_quota_manager.get_quota.assert_called_with("secure_tenant", "chat_requests_per_hour")
