...
def search(req: SearchRequest, request: Request) -> dict:
    ...
    if req.top_k < MIN_TOP_K or req.top_k > MAX_TOP_K:
        raise HTTPException(
            status_code=HTTP_STATUS_BAD_REQUEST, detail="top_k invalide"
        )
    if req.offset < 0:
        raise HTTPException(
            status_code=HTTP_STATUS_BAD_REQUEST, detail="offset invalide"
        )
    ...
    try:
        ...
        if exc.status_code in (
            ...
            raise HTTPException(
                status_code=exc.status_code, detail="invalid request"
            ) from exc
        raise HTTPException(
            status_code=HTTP_STATUS_BAD_REQUEST, detail="client error"
        ) from exc
